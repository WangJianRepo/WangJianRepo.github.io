...
## 5 结束语
&emsp;&emsp;视频采样可以作为视频拷贝检测、视频检索等应用中对视频降维的有效手段。本文从视频采样实际需求出发，对视频的编解码技术做了简要介绍，介绍了I帧、P帧和B帧，提出将I帧的位置作为镜头分割点，I帧作为镜头关键帧；然后，给出了利用FFMPEG在解码和非解码的情况下提取I帧的方法；最后对该方法做了进一步的讨论。欢迎大家在页面下方进行讨论。
